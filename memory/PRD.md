...
- ✅ **"Bank Balance" column left source-driven (Feb 2026)** — Design correction: this column reflects a balance-after value that comes with the *source data* (mock/seed onboarding rows, and eventually Veryfi-OCR'd bank statement lines which print "Balance $X" per row). Plaid doesn't provide per-transaction balances, so Plaid-imported rows correctly show "—" until a statement OCR carries the actual bank-printed value. We deliberately do NOT compute running balances client-side — Cash-on-Hand on the dashboard already summarizes ledger balance across all sources, and per-row computed balances would drift out-of-sync with the true statement.
