...
- **Advanced Features Foundation — Phase 1 (2026-02)** — Invisible foundation for Classes / Projects / Budgets. `companies.features` sub-doc with three OFF-by-default flags (`classes_enabled`, `projects_enabled`, `budgets_enabled`). New collections skeleton: `classes`, `projects`, `project_phases`, `budgets`, `budget_lines`. All nullable FKs (`class_id`, `project_id`, `phase_id`, plus `qbo_legacy_tags` for 2028-sunset Tags capture) added to `transactions`, `invoices`, `bills`, `payments`, `receipts`, `estimates`, and `journal_entries.lines`. Sparse indexes on every FK, plus a unique `(budget_id, account_id, period_key)` index. `reports.py::_signed_balances` gains optional `class_id` / `project_id` filters (default None → zero query cost change). Startup hook backfills `features` on legacy companies. Zero UI change — Phase 2 (Classes UI) will ship as pure frontend work. See `/app/backend/advanced_features.py` for the full taxonomy. Regression: 7 pytest scenarios (default flags, legacy read, idempotent backfill, unfiltered no-op, txn-level filter, JE-line filter, index sanity) in `tests/test_advanced_features_phase1.py`.
