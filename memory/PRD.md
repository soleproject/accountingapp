...
- ✅ **Bank Balance column fix (Feb 2026)** — Transactions page's "Bank Balance" column was showing "—" for every Plaid-imported row. Root cause: `bank_balance_after` was only set by the mock/seed onboarding path — real Plaid ingest never populated it. Fixes: new `plaid_connect._refresh_bank_balances_for_account(cid, bank_id)` recomputes running balance for every posted txn on a bank account, seeded by the sum of journal-entry lines hitting that account (opening balance + reclassifications), ordered `date ASC, created_at ASC`, idempotent. Called automatically at the end of every `categorize_and_insert_plaid_txns` batch. `tests/backfill_bank_balance_after.py` one-shot backfill retroactively populated 2,002 pre-existing rows (Marketing Co 122 + Bright Beans 10 + 739 LLC 1,870). Last row's balance now equals Cash-on-Hand on the dashboard.
