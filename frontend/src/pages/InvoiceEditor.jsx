        ...
        if (editMode) {
          ...
          const loadedLines = (inv.line_items || []).length
            ? inv.line_items.map(l => ({
                ...
                category: l.category || "",
                // Preserve per-line sales tax so it survives a refresh.
                // Without these, the dropdown resets to "Select a tax"
                // and the line-tax rollup silently disappears.
          ...
          // `inv.tax` on disk is the ROLLED-UP figure (doc-level tax
          // input + Σ per-line tax). computeTotals re-adds the line
          // rollup for display, so we must peel that off here or the
          // displayed / persisted tax doubles on every save cycle.
            ...
            <>
              <div className="flex items-center justify-between gap-3">
                ...
                <span
                  ...
                  data-testid="invoice-editor-tax-auto"
              ...
              {(Number(tax) !== 0 || showTaxAdj) ? (
                <div className="flex items-center justify-between gap-3">
                  <span className="text-sm text-slate-500">
                    ...
                    <button
                      ...
                      data-testid="invoice-editor-tax-adjust-clear"
                    >clear</button>
                  </span>
                  <input
                ...
                <div className="flex items-center justify-end">
                  <button
                    ...
                    data-testid="invoice-editor-tax-adjust-toggle"
...
function CreateTaxDialog({ onClose, onCreated, currentId }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // Backend returns `{tax: {...full doc}}`. Fall back to a
      // synthesized stub so the parent's setTaxes-and-apply logic
      // never sees `undefined` (would crash on .sort → .name).
