...
export default function Bills() {
  ...
  const load = async () => {
    ...
    setItems(b.data.bills || []); setContacts(c.data.contacts || []);
  };
      ...
      <div className="rounded-xl border bg-white overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-xs uppercase text-slate-500 border-b">
            <tr>
              <th className="px-3 py-2 text-left">Number</th><th className="px-3 py-2 text-left">Vendor</th>
          ...
          <tbody>
            {items.map(b => (
              <tr key={b.id} className="border-b hover:bg-slate-50">
                ...
                <td className="px-3 py-2 text-right"><button onClick={() => del(b.id)} className="text-red-500 p-1"><Trash2 size={13} /></button></td>
              </tr>
            ))}
...
function BillModal({ contacts, currentId, onClose }) {
  ...
  const save = async () => {
    ...
    await api.post(`/companies/${currentId}/bills`, {
      contact_id: contact || null, contact_name: c?.name || "",
      issue_date: issue, due_date: due, line_items: lines, status: "open",
    });
    toast.success("Bill created"); onClose();
  };
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl p-5 space-y-3">
        <div className="flex items-center justify-between"><h3 className="font-heading font-semibold">New Bill</h3><button onClick={onClose}><X size={16} /></button></div>
        <div className="grid grid-cols-3 gap-2">
          <select value={contact} onChange={(e) => setContact(e.target.value)} className="border rounded px-2 py-1.5 text-sm">
            <option value="">Vendor…</option>
            {contacts.filter(c => c.type === "vendor").map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
        ...
        <div className="flex justify-end gap-4 items-center border-t pt-3">
          ...
          <button data-testid={TID.saveBtn} onClick={save} className="px-4 py-2 rounded-md bg-slate-900 text-white text-sm">Save Bill</button>
        </div>
      </div>
    </div>
  );
