...
import type { CreateBillState } from '../_actions/createBill';
...
let nextId = 0;
const newLine = (): Line => ({ id: ++nextId, description: '', quantity: '1', unitPrice: '', expenseAccountId: '' });
...
interface Props {
  ...
  apAccounts: Account[];
  action: (
    prev: CreateBillState | undefined,
    formData: FormData,
  ) => Promise<CreateBillState | undefined>;
  initial?: BillFormInitial;
  submitDraftLabel?: string;
  submitPostLabel?: string;
}

const seedLines = (initial?: BillFormInitial): Line[] => {
...
export function BillForm({
  ...
  const [state, formAction, pending] = useActionState<CreateBillState | undefined, FormData>(action, undefined);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-5">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium uppercase tracking-wide text-zinc-500">Bill number</label>
          <input name="billNumber" defaultValue={initial?.billNumber ?? ''} placeholder="Optional" className="rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium uppercase tracking-wide text-zinc-500">AP account</label>
          <select name="apAccountId" required defaultValue={initial?.apAccountId ?? ''} className="rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900">
            <option value="">— Select —</option>
            {apAccounts.map((a) => <option key={a.id} value={a.id}>{a.accountNumber} · {a.accountName}</option>)}
          </select>
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium uppercase tracking-wide text-zinc-500">Bill date</label>
          <input name="billDate" type="date" required defaultValue={initial?.billDate ?? new Date().toISOString().slice(0, 10)} className="rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900" />
        </div>
        <div className="flex flex-col gap-1">
        ...
        <table className="w-full text-sm">
          <thead className="bg-zinc-50 dark:bg-zinc-900">
            <tr>
              ...
              <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wide text-zinc-500">Expense account</th>
            ...
            {lines.map((line, idx) => {
              ...
              return (
                <tr key={line.id} className="border-t border-zinc-100 dark:border-zinc-800">
                  <td className="p-2"><input name={`lines[${idx}].description`} value={line.description} onChange={(e) => update(line.id, { description: e.target.value })} className="w-full rounded-md border border-zinc-300 bg-white px-2 py-1 text-sm dark:border-zinc-700 dark:bg-zinc-900" /></td>
                  <td className="p-2">
                    <select name={`lines[${idx}].expenseAccountId`} value={line.expenseAccountId} onChange={(e) => update(line.id, { expenseAccountId: e.target.value })} required className="w-full rounded-md border border-zinc-300 bg-white px-2 py-1 text-sm dark:border-zinc-700 dark:bg-zinc-900">
                      <option value="">— Select —</option>
                      {expenseAccounts.map((a) => <option key={a.id} value={a.id}>{a.accountNumber} · {a.accountName}</option>)}
                    </select>
                  </td>
                  <td className="p-2"><input name={`lines[${idx}].quantity`} type="number" step="0.01" min="0" value={line.quantity} onChange={(e) => update(line.id, { quantity: e.target.value })} required className="w-24 rounded-md border border-zinc-300 bg-white px-2 py-1 text-right text-sm tabular-nums dark:border-zinc-700 dark:bg-zinc-900" /></td>
                  <td className="p-2"><input name={`lines[${idx}].unitPrice`} type="number" step="0.01" min="0" value={line.unitPrice} onChange={(e) => update(line.id, { unitPrice: e.target.value })} required className="w-28 rounded-md border border-zinc-300 bg-white px-2 py-1 text-right text-sm tabular-nums dark:border-zinc-700 dark:bg-zinc-900" /></td>
                  <td className="p-2 text-right tabular-nums">${amount.toFixed(2)}</td>
                  <td className="p-2 text-center"><button type="button" onClick={() => setLines((cur) => (cur.length > 1 ? cur.filter((l) => l.id !== line.id) : cur))} disabled={lines.length <= 1} className="text-zinc-400 hover:text-red-600 disabled:opacity-30">×</button></td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button type="button" onClick={() => setLines((cur) => [...cur, newLine()])} className="rounded-md border border-zinc-300 px-3 py-1.5 text-sm hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-900">+ Add line</button>
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" name="postNow" checked={postNow} onChange={(e) => setPostNow(e.target.checked)} value="true" />
          Post JE now (debit expense / credit AP)
        </label>
