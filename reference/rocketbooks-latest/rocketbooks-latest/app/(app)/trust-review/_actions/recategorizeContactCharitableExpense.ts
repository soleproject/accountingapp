...
export interface RecategorizeContactCharitableExpenseResult {
	ok: boolean;
	processed: number;
...
/**
 * Per-contact bulk action for "this isn't actually a charitable
 * contribution". Reverses each finding's JE and reposts with the 515
 * line replaced by a line on `targetAccountId`. Inserts a
 * TRUST_NON_TRUST_RECATEGORIZED audit per JE; dismisses findings.
 *
 * No typeTag stamping — there's no "non-charity vendor" classification.
 */
export async function recategorizeContactCharitableExpense(args: {
	findingIds: string[];
	targetAccountId: string;
}): Promise<RecategorizeContactCharitableExpenseResult> {
...
async function recategorizeOneFinding(args: {
	...
	if (finding.code !== 'TRUST_515_VERIFY_501C3') {
		return { ok: false, error: `recategorizeContactCharitableExpense doesn't apply to ${finding.code}` };
	}

	// Find the 515 line on this JE (detail_type='charitable_contributions').
	...
	if (!sourceLineSummary) {
		return { ok: false, error: 'No 515 line on this JE' };
	}
	...
	if (sourceLines.length === 0) {
		return { ok: false, error: 'No matching 515 line on this JE' };
	}
	...
	try {
		await db.transaction(async (tx) => {
			await reverseJournalEntry(
				{
					...
					reversalMemo: `Reversal — 515 line recategorized to ${target.accountNumber ?? ''} ${target.accountName} (not a charitable contribution)`,
				},
				tx,
			);
			...
			await tx.insert(trustReviewFindings).values({
				...
				severity: 'warn',
				message: `Moved from ${sourceLineSummary.accountNumber ?? ''} ${sourceLineSummary.accountName} to ${target.accountNumber ?? ''} ${target.accountName} — not a charitable contribution.`,
				metadata: {
					accountId: target.id,
			...
			await tx
				...
				.set({
					...
					dismissedNote: `Auto-dismissed: recategorized to ${target.accountNumber ?? ''} ${target.accountName} — not a charitable contribution.`,
