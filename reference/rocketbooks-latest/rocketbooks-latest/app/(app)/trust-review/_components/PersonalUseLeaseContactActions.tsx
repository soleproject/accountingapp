...
const CYAN_F = 'border-cyan-300 bg-cyan-50 text-cyan-700 hover:bg-cyan-100 dark:border-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-300 dark:hover:bg-cyan-900/50';
const CYAN_M = 'border-zinc-200 bg-transparent text-zinc-400 hover:border-cyan-300 hover:bg-cyan-50 hover:text-cyan-700 dark:border-zinc-700 dark:text-zinc-500';
...
/**
 * Per-contact actions for TRUST_DEFERRED_PERSONAL_USE_LEASE.
 *
 *   ⊘ Not Personal-Use Lease → Other → CoA picker
 *   📄 Request Lease Resolution → per-row Link to /trust-documents/new
 *   ✓ Mark Configured          → confirmFindingAsIs (lease set up externally)
 */
export function PersonalUseLeaseContactActions({
	...
	const runOther = (id: string) => {
		...
		startTransition(async () => {
			const r = await recategorizeFindingToAccount({
				...
				applicableCodes: ['TRUST_DEFERRED_PERSONAL_USE_LEASE'],
				sourceLine: { kind: 'metadata_account_id' },
				auditVerb: 'personal-use lease income',
			});
	...
	const runConfirm = () => {
		...
		startTransition(async () => {
			const r = await confirmFindingAsIs({
				findingIds,
				applicableCodes: ['TRUST_DEFERRED_PERSONAL_USE_LEASE'],
				auditCode: 'TRUST_PERSONAL_USE_LEASE_CONFIGURED',
				auditMessage: `Personal-use lease for ${contactName} confirmed configured externally.`,
			});
			if (!r.ok) setError(r.error ?? `${r.processed} ok, ${r.failed.length} failed`);
		});
	};
		...
		<div className="flex items-center gap-2">
			<div ref={notRef} className="relative">
				<button type="button" onClick={() => setNotOpen((v) => !v)} disabled={disabled}
					title={`Not personal-use lease — recategorize all ${findingIds.length}`}
			...
			{findingIds.length === 1 && (
				<Link href={`/trust-documents/new?template=personal-use-lease&fromFinding=${encodeURIComponent(findingIds[0])}`}
					title="Draft personal-use lease resolution"
					className={`${ICON_BASE} ${muted ? CYAN_M : CYAN_F}`}>
					<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
			...
			<button type="button" onClick={runConfirm} disabled={disabled}
				title={`Mark lease as configured externally for all ${findingIds.length}`}
