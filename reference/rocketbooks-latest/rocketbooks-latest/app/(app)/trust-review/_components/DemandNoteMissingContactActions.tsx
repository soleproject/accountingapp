...
/**
 * Per-contact actions for TRUST_DEMAND_NOTE_MISSING_NOTE.
 *
 *   ⊘ Not a Demand Note  → Other → CoA picker
 *   ✍️ Draft Promissory   → per-row Link to /trust-documents/new
 *   ✓ Acknowledge On File → confirmFindingAsIs (note exists off-system)
 */
export function DemandNoteMissingContactActions({
	...
	const runOther = (id: string) => {
		...
		startTransition(async () => {
			const r = await recategorizeFindingToAccount({
				...
				applicableCodes: ['TRUST_DEMAND_NOTE_MISSING_NOTE'],
				sourceLine: { kind: 'metadata_account_id' },
				auditVerb: 'a demand-note posting',
			});
	...
	const runConfirm = () => {
		...
		startTransition(async () => {
			const r = await confirmFindingAsIs({
				findingIds,
				applicableCodes: ['TRUST_DEMAND_NOTE_MISSING_NOTE'],
				auditCode: 'TRUST_DEMAND_NOTE_CONFIRMED',
				auditMessage: `Promissory note confirmed on file (off-system) for ${contactName}.`,
			});
			if (!r.ok) setError(r.error ?? `${r.processed} ok, ${r.failed.length} failed`);
		});
	};
		...
		<div className="flex items-center gap-2">
			<div ref={notRef} className="relative">
				<button type="button" onClick={() => setNotOpen((v) => !v)} disabled={disabled}
					title={`Not a demand note — recategorize all ${findingIds.length}`}
			...
			{findingIds.length === 1 && (
				<Link href={`/trust-documents/new?template=promissory-note&fromFinding=${encodeURIComponent(findingIds[0])}`}
					title="Draft promissory note PDF for this demand-note line"
			...
			<button type="button" onClick={runConfirm} disabled={disabled}
				title={`Acknowledge: a promissory note exists on file for ${contactName}`}
