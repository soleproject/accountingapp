...
import { invoices, invoiceLines, contacts, organizations, payments } from '@/db/schema/schema';
...
/**
 * Render an arbitrary contact.address JSON value as a multi-line block.
 * Address shape isn't pinned down at the schema level so we accept the
 * common formats (a single string, or an object with line1/city/state/zip
 * variants).
 */
...
function StatusPill({ label }: { label: string }) {
  const cls = STATUS_PALETTE[label] ?? STATUS_PALETTE.open;
  return (
...
export default async function InvoiceDetailPage({ params }: PageProps) {
  ...
  const isOverdue = !!inv.dueDate && inv.dueDate < today && balanceDue > 0;
  const displayStatus = !inv.posted
    ? 'draft'
  ...
  const invoiceLabel = inv.number ?? `#${inv.id.slice(0, 8)}`;
  const billToName = inv.contactCompany || inv.contactName || '—';
  const addressLines = renderAddress(inv.contactAddress);

  return (
    <div className="flex flex-col gap-4">
      {/* Action bar — kept outside the "paper" so the printable doc looks
          clean. */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link href="/invoices" className="text-sm text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300">
          ← Back to invoices
        </Link>
        <div className="flex items-center gap-2">
          ...
          <DeleteInvoiceButton invoiceId={inv.id} invoiceLabel={invoiceLabel} />
        </div>
      </div>
      ...
      <article className="mx-auto w-full max-w-3xl rounded-lg border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-950 sm:p-10">
        ...
        <section className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div>
            <div className="text-xs font-medium uppercase tracking-wide text-zinc-500">Bill to</div>
            <div className="mt-2 text-sm">
              <div className="font-medium text-zinc-900 dark:text-zinc-100">{billToName}</div>
              {inv.contactCompany && inv.contactName && inv.contactName !== inv.contactCompany && (
                <div className="text-zinc-700 dark:text-zinc-300">{inv.contactName}</div>
              )}
              {addressLines.map((l, i) => (
                <div key={i} className="text-zinc-700 dark:text-zinc-300">{l}</div>
              ))}
              {inv.contactEmail && (
                <div className="mt-1 text-zinc-600 dark:text-zinc-400">{inv.contactEmail}</div>
              )}
              {inv.contactPhone && (
                <div className="text-zinc-600 dark:text-zinc-400">{inv.contactPhone}</div>
              )}
            </div>
          </div>
          <div className="text-sm">
            <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5">
              ...
              <dd className="text-zinc-900 dark:text-zinc-100">{invoiceLabel}</dd>
              <dt className="text-xs font-medium uppercase tracking-wide text-zinc-500">Date</dt>
              <dd className="text-zinc-900 dark:text-zinc-100">{formatLongDate(inv.date)}</dd>
              <dt className="text-xs font-medium uppercase tracking-wide text-zinc-500">Due</dt>
              <dd className="text-zinc-900 dark:text-zinc-100">{formatLongDate(inv.dueDate)}</dd>
              {inv.journalEntryId && (
                <>
                  <dt className="text-xs font-medium uppercase tracking-wide text-zinc-500">Journal</dt>
                  <dd>
                    <Link href={`/journal-entries/${inv.journalEntryId}`} className="text-blue-600 underline dark:text-blue-400">
                      View JE
                    </Link>
                  </dd>
                </>
              ...
              <tbody>
                {paymentRows.map((p) => (
                  <tr key={p.id} className="border-t border-zinc-100 dark:border-zinc-800/60">
                    ...
                    <td className="py-2 text-zinc-700 dark:text-zinc-300">Customer payment</td>
                    <td className="py-2">
                      {p.journalEntryId ? (
        ...
        {inv.memo && (
          <section className="mt-8 border-t border-zinc-200 pt-6 dark:border-zinc-800">
            ...
            <p className="whitespace-pre-wrap text-sm text-zinc-700 dark:text-zinc-300">{inv.memo}</p>
          </section>
        )}
      </article>
    </div>
