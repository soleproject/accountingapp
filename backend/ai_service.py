...
CONTACT_EXTRACTION_SYSTEM = (
    ...
    "Rules:\n"
    "- DEFAULT to has_counterparty=true and extract a clean name.\n"
    "- has_counterparty=false ONLY when the description is genuinely internal — no third-party entity name. Signals: bare account numbers after TO/FROM ('TRANSFER TO ACCT 6084'), monthly bank fees, interest earned/paid, and no ORIG:/Bnf=/Org= field with a real name.\n"
    "- 'Zelle payment to Romeo Ugali Conf# xxxx' → extracted_name='Romeo Ugali' (the recipient, NOT the app).\n"
    "- 'WT Fed#... ORIG:ACME LLC ID:...' → extracted_name='Acme LLC'.\n"
    "- 'Recurring Payment authorized on 12/26 Zoom.Com 888-799 Zoom.Us CA S3553 Card 6236' → extracted_name='Zoom'.\n"
    "- 'CAPITAL ONE DES:MOBILE PMT ID:...' → extracted_name='Capital One'.\n"
    "- Strip authorization codes, transaction ids, card numbers, memos, location codes.\n"
    "- extracted_name must be a CLEAN counterparty ≤ 60 chars. If you can't extract a clean name, set has_counterparty=false.\n"
    "- Match semantically to existing contacts: 'PayPal' matches 'PayPal Inc.', 'Capital One' matches 'Capital One, NA'.\n"
    "- Only set match_existing_id when confident it's the same entity. If uncertain, null and let a new contact be created.\n"
    "- NEVER return the raw bank description as extracted_name."
)
