...
async def _push_one_estimate(company_id: str, realm_id: str,
                               doc: dict) -> tuple[str | None, dict | None]:
    body = await _estimate_body(company_id, doc)
    resp = await _post(company_id, realm_id,
                        f"/company/{realm_id}/estimate", body)
    twin = resp.get("Estimate") or {}
    new_id = twin.get("Id")
    twin_patch = _local_patch_from_qbo_estimate(twin) if new_id else None
    return (str(new_id) if new_id else None, twin_patch)


async def _push_one_po(company_id: str, realm_id: str,
                        doc: dict) -> tuple[str | None, dict | None]:
    body = await _po_body(company_id, doc)
    resp = await _post(company_id, realm_id,
                        f"/company/{realm_id}/purchaseorder", body)
    twin = resp.get("PurchaseOrder") or {}
    new_id = twin.get("Id")
    twin_patch = _local_patch_from_qbo_po(twin) if new_id else None
    return (str(new_id) if new_id else None, twin_patch)
...
async def _push_one_sales_receipt(company_id: str, realm_id: str,
                                    doc: dict) -> tuple[str | None, dict | None]:
    body = await _sales_receipt_body(company_id, doc)
    resp = await _post(company_id, realm_id,
                        f"/company/{realm_id}/salesreceipt", body)
    twin = resp.get("SalesReceipt") or {}
    new_id = twin.get("Id")
    twin_patch = _local_patch_from_qbo_sales_receipt(twin) if new_id else None
    return (str(new_id) if new_id else None, twin_patch)


async def _push_one_deposit(company_id: str, realm_id: str,
                              doc: dict) -> tuple[str | None, dict | None]:
    body = await _deposit_body(company_id, doc)
    resp = await _post(company_id, realm_id,
                        f"/company/{realm_id}/deposit", body)
    twin = resp.get("Deposit") or {}
    new_id = twin.get("Id")
    twin_patch = _local_patch_from_qbo_deposit(twin) if new_id else None
    return (str(new_id) if new_id else None, twin_patch)


async def _push_one_credit_memo(company_id: str, realm_id: str,
                                  doc: dict) -> tuple[str | None, dict | None]:
    body = await _credit_memo_body(company_id, doc)
    resp = await _post(company_id, realm_id,
                        f"/company/{realm_id}/creditmemo", body)
    twin = resp.get("CreditMemo") or {}
    new_id = twin.get("Id")
    twin_patch = _local_patch_from_qbo_credit_memo(twin) if new_id else None
    return (str(new_id) if new_id else None, twin_patch)


async def _push_one_refund_receipt(company_id: str, realm_id: str,
                                     doc: dict) -> tuple[str | None, dict | None]:
    body = await _refund_receipt_body(company_id, doc)
    resp = await _post(company_id, realm_id,
                        f"/company/{realm_id}/refundreceipt", body)
    twin = resp.get("RefundReceipt") or {}
    new_id = twin.get("Id")
    twin_patch = _local_patch_from_qbo_refund_receipt(twin) if new_id else None
    return (str(new_id) if new_id else None, twin_patch)
...
_ENTITY_META = {
    ...
    "sales_receipt":  ("salesreceipt", "SalesReceipt",  "transactions", None),
...
async def _run_auto_update(company_id: str, entity: str,
    ...
    try:
        ...
        if entity == "account":
            ...
            # Full-replace update for Estimate — same rationale as
            # invoice/bill (line changes must round-trip).
            try:
                body = await _estimate_body(company_id, doc)
            except ValueError as ve:
                await append_log(
                    company_id, "autoupdate",
                    f"Auto-update estimate {doc_id} skipped: {ve}",
                    {"entity": entity, "doc_id": doc_id,
                     "reason": str(ve)})
                return
            ...
            try:
                ...
                await append_log(
                    company_id, "autoupdate",
                    f"Auto-update purchase_order {doc_id} skipped: {ve}",
                    {"entity": entity, "doc_id": doc_id,
                     "reason": str(ve)})
                return
            ...
            try:
                ...
                await append_log(
                    ...
                    {"entity": entity, "doc_id": doc_id,
                     "reason": str(ve)})
                return
        elif entity == "sales_receipt":
            try:
                body = await _sales_receipt_body(company_id, doc)
            except ValueError as ve:
                await append_log(
                    company_id, "autoupdate",
                    f"Auto-update sales_receipt {doc_id} skipped: {ve}",
                    {"entity": entity, "doc_id": doc_id,
                     "reason": str(ve)})
                return
        elif entity == "deposit":
            try:
                ...
                await append_log(
                    company_id, "autoupdate",
                    f"Auto-update deposit {doc_id} skipped: {ve}",
                    {"entity": entity, "doc_id": doc_id,
                     "reason": str(ve)})
                return
        elif entity == "credit_memo":
            try:
                body = await _credit_memo_body(company_id, doc)
            except ValueError as ve:
                await append_log(
                    company_id, "autoupdate",
                    f"Auto-update credit_memo {doc_id} skipped: {ve}",
                    {"entity": entity, "doc_id": doc_id,
                     "reason": str(ve)})
                return
            ...
            try:
                ...
                await append_log(
                    company_id, "autoupdate",
                    f"Auto-update refund_receipt {doc_id} skipped: {ve}",
                    {"entity": entity, "doc_id": doc_id,
                     "reason": str(ve)})
                return
        ...
        if entity == "invoice":
            ...
            twin = resp.get("Estimate") or {}
            if twin:
                set_patch = {**_local_patch_from_qbo_estimate(twin),
                             **set_patch}
        elif entity == "purchase_order":
            twin = resp.get("PurchaseOrder") or {}
            if twin:
            ...
            twin = resp.get("SalesReceipt") or {}
            if twin:
                set_patch = {**_local_patch_from_qbo_sales_receipt(twin),
                             **set_patch}
        elif entity == "deposit":
            twin = resp.get("Deposit") or {}
            if twin:
                set_patch = {**_local_patch_from_qbo_deposit(twin),
                             **set_patch}
        elif entity == "credit_memo":
            twin = resp.get("CreditMemo") or {}
            if twin:
                set_patch = {**_local_patch_from_qbo_credit_memo(twin),
                             **set_patch}
        elif entity == "refund_receipt":
            twin = resp.get("RefundReceipt") or {}
            if twin:
...
_HANDLERS = {
    ...
    "sales_receipt":  ("transactions", _push_one_sales_receipt, None),
