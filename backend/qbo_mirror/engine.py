...
# ─── Estimate / Purchase Order normalizers (Phase 3) ───────────────
...
def _norm_estimate_qbo(o: dict) -> dict:
    number = (o.get("DocNumber") or "").strip() or f"EST-{o.get('Id')}"
    return {
        "qbo_id": o.get("Id"),
        "natural_key": f"est::{number.lower()}",
        "number": number,
...
def _norm_po_qbo(o: dict) -> dict:
    number = (o.get("DocNumber") or "").strip() or f"PO-{o.get('Id')}"
    return {
        "qbo_id": o.get("Id"),
        "natural_key": f"po::{number.lower()}",
        "number": number,
...
# ─── CreditMemo / RefundReceipt normalizers (Phase 4c) ─────────────

def _norm_credit_memo_local(t: dict) -> dict:
    number = (t.get("number") or "").strip()
    if not number and t.get("qbo_id"):
        number = f"CreditMemo-{t.get('qbo_id')}"
    return {
        "qbo_id": t.get("qbo_id"),
        "natural_key": f"cm::{number.lower()}",
        "number": number,
        "date": t.get("date") or "",
        "total": round(abs(float(t.get("amount") or 0)), 2),
    }
...
def _norm_credit_memo_qbo(o: dict) -> dict:
    number = (o.get("DocNumber") or "").strip() \
        or f"CreditMemo-{o.get('Id')}"
    return {
        "qbo_id": o.get("Id"),
        "natural_key": f"cm::{number.lower()}",
        "number": number,
...
def _norm_refund_receipt_local(t: dict) -> dict:
    number = (t.get("number") or "").strip()
    if not number and t.get("qbo_id"):
        number = f"RefundReceipt-{t.get('qbo_id')}"
    return {
        "qbo_id": t.get("qbo_id"),
        "natural_key": f"rr::{number.lower()}",
        "number": number,
        "date": t.get("date") or "",
        "total": round(abs(float(t.get("amount") or 0)), 2),
    }
...
def _norm_refund_receipt_qbo(o: dict) -> dict:
    number = (o.get("DocNumber") or "").strip() \
        or f"RefundReceipt-{o.get('Id')}"
    return {
        "qbo_id": o.get("Id"),
        "natural_key": f"rr::{number.lower()}",
        "number": number,
...
_DRIFT_FIELDS = {
    ...
    "journal_entries": [],
    # Estimates / POs: same 3-way as invoices/bills but without
        ...
        return [_norm_deposit_local(t)
                async for t in db.transactions.find(
                    {"company_id": company_id, "txn_type": "Deposit",
                      "voided": {"$ne": True}})]
    if entity == "credit_memos":
        return [_norm_credit_memo_local(t)
                async for t in db.transactions.find(
                    {"company_id": company_id, "txn_type": "CreditMemo",
                      "voided": {"$ne": True}})]
    if entity == "refund_receipts":
        return [_norm_refund_receipt_local(t)
                async for t in db.transactions.find(
...
async def _fetch_qbo(company_id: str, realm_id: str,
    ...
    if entity == "estimates":
        return [_norm_estimate_qbo(o)
                async for o in Q.query_all(company_id, realm_id, "Estimate")]
    if entity == "purchase_orders":
    ...
    if entity == "credit_memos":
        return [_norm_credit_memo_qbo(o)
                async for o in Q.query_all(company_id, realm_id, "CreditMemo")]
    if entity == "refund_receipts":
