...
async def sync_and_import(cid: str, item: dict, selected_account_ids: list[str] | None = None) -> int:
    ...
    # NOTE: Opening balance JEs used to be retried here via a 30-day
    # history proxy. The proper trigger is Plaid's HISTORICAL_UPDATE
    # webhook, which is now handled directly in
    # `sync_tasks._run_sync -> _post_deferred_plaid_opening_balances`.
    # This webhook-driven codepath (used by the Plaid `/webhook` endpoint
    # to import mid-stream deltas) intentionally does NOT touch OBE JEs.
