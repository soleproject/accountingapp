...
async def upsert(coll: str, doc: dict) -> None:
    """Idempotent upsert by (company_id, source, qbo_id). Sets a
    deterministic `id` on insert so re-runs never fabricate new UUIDs
    for the same QBO row — downstream APIs can query the record by
    that stable id."""
    key = {"company_id": doc["company_id"], "source": "qbo",
           "qbo_id": doc["qbo_id"]}
    doc.setdefault("id", f"qbo-{doc['qbo_id']}")
